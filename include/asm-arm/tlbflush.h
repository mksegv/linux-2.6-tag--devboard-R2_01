 ...
 *	  v4uwt - ARMv4 with write buffer without separate I/D TLB or entry instructions
...
/* I, do it, myyyyyyyyyyyy   waaaaaaaaaaaaayyyyyyyyyyyyy */
#include <asm/arch/tlbflush.h>
#else
...
flush_tlb_page(struct vm_area_struct *vma, unsigned long uaddr)
{
	...
	if (cpu_isset(smp_processor_id(), vma->vm_mm->cpu_vm_mask)) {
		...
		if (!tlb_flag(TLB_V4_I_PAGE) && tlb_flag(TLB_V4_I_FULL))
			asm("mcr%? p15, 0, %0, c8, c5, 0" : : "r" (zero));
		if (!tlb_flag(TLB_V4_U_PAGE) && tlb_flag(TLB_V4_U_FULL))
			asm("mcr%? p15, 0, %0, c8, c7, 0" : : "r" (zero));
	}
