...
# Default arch to cris, but pick up ARCH from the make arguments if
# packages/os/linux-2.6 is building this
...
CROSS_COMPILE   := $(shell dirname `gcc-cris -mlinux -march=v32 -print-prog-name=as`)/
$(shell ln -sfn $(srctree)/arch/cris/arch-v32 $(srctree)/arch/cris/arch)
endif
ifdef CONFIG_ETRAX_ARCH_V10
CROSS_COMPILE   := $(shell dirname `gcc-cris -mlinux -print-prog-name=as`)/
$(shell ln -sfn $(srctree)/arch/cris/arch-v10 $(srctree)/arch/cris/arch)
endif
ifdef CONFIG_ARM
CROSS_COMPILE := arm-linux-
# I didnt get the line below to give the right path, hence the hardcoded one.
#CROSS_COMPILE   := $(shell dirname `arm-linux-gcc -print-prog-name=as`)/
endif
...
NOSTDINC_FLAGS  = -nostdinc -iprefix $(INCLUDE_PREFIX) -iwithprefix include
