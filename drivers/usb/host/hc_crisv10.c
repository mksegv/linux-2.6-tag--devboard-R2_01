...
static const char *usb_hcd_version = "$Revision: 1.4 $";
...
static void etrax_usb_add_to_ctrl_sb_list(struct urb *urb, int epid)
{
	...
	if (usb_pipeout(urb->pipe)) {
		...
		if ((0 < urb->transfer_buffer_length) && urb->transfer_buffer) {
...
static struct device_driver etrax_usb_driver = {
	.name =		(char *) "ETRAX 100LX",
};
...
static int __init etrax_usb_hc_init(void)
{
	...
	if (cris_request_io_interface(if_usb_1, "ETRAX100LX USB-HCD")) {
		printk(KERN_CRIT "usb-host: request IO interface usb%d failed", 1);
		return -EBUSY;
	}
#endif
...
#endif
		printk(KERN_CRIT "usb-host: request IO interface usb%d failed", 2);
		return -EBUSY;
	}
#endif
	...
	*R_USB_EPT_DATA = (IO_STATE(R_USB_EPT_DATA, valid, no) |
	...
	init_tx_isoc_ep();
        ...
        kobject_set_name(&etrax_usb_driver.kobj, "etrax_usb_driver");
        kobject_register(&etrax_usb_driver.kobj);
	hc->bus->controller->driver = &etrax_usb_driver;
...
static void __exit etrax_usb_hc_cleanup(void)
{
	DBFENTER;
