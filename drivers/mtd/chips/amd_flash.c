...
static inline map_word wide_read(struct map_info *map, __u32 addr)
{
	return map_read(map, addr);
}
				...
				int interleave)
{
	...
	if ((interleave == 2) && (map->bankwidth == 4)) {
		...
		read1 = wide_read(map, addr).x[0];
		read2 = wide_read(map, addr).x[0];
	...
	return ((wide_read(map, addr).x[0] & D6_MASK) !=
		(wide_read(map, addr).x[0] & D6_MASK));
}
				   ...
				   unsigned long sect_addr)
{
	...
	status = wide_read(map, sect_addr + (map->bankwidth * ADDR_SECTOR_LOCK)).x[0];
	wide_write(map, CMD_RESET_DATA, 0);
	return status;
}
			  ...
			  const struct amd_flash_info *table, int table_size)
{
	...
	mfr_id = wide_read(map, base + (map->bankwidth * ADDR_MANUFACTURER)).x[0];
	dev_id = wide_read(map, base + (map->bankwidth * ADDR_DEVICE_ID)).x[0];
...
retry:
	...
	if (!times_left) {
		...
		__u32 verify;
		if ((verify = wide_read(map, adr).x[0]) != datum) {
...
retry:
	...
	/* Verify every single word */
	{
		...
		for (address = adr; address < (adr + size); address++) {
			if ((verify = map_read(map, address).x[0]) != 0xFF) {
				error = 1;
				break;
			}
		}
