/* $Id: ethernet.c,v 1.33 2004/12/13 12:21:52 starvik Exp $
 *
 ...
 *
 * $Log: ethernet.c,v $
 * Revision 1.33  2004/12/13 12:21:52  starvik
...
e100rxtx_interrupt(int irq, void *dev_id, struct pt_regs * regs)
{
	...
	*R_VECT_MASK_CLR =
	  IO_STATE(R_VECT_MASK_CLR, dma0, clr) |
	  IO_STATE(R_VECT_MASK_CLR, dma1, clr);
...
e100_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	switch (cmd) {
		case SIOCETHTOOL:
			ret = e100_ethtool_ioctl(dev,ifr);
			break;
...
e100_ethtool_ioctl(struct net_device *dev, struct ifreq *ifr)
{
	...
	switch (ecmd.cmd) {
		...
		case ETHTOOL_GDRVINFO:
		{
			...
			strncpy(info.version, "$Revision: 1.33 $", sizeof(info.version) - 1);
