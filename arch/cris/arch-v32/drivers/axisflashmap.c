...
static struct mtd_partition nand_partitions[1] = {
       {
		.name = "part0",
		.size = 0,
		.offset = 0
...
#ifdef CONFIG_ETRAX_NANDFLASH
	mtd_nand = crisv32_nand_flash_probe();
#endif
...
#ifdef CONFIG_ETRAX_NANDFLASH
		nand_partitions[0].size = mtd_nand->size;
                err = add_mtd_partitions(mtd_nand, nand_partitions, 1);
#endif
