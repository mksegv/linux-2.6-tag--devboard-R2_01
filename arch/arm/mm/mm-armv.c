...
static struct mem_types mem_types[] __initdata = {
	...
	[MT_HIGH_VECTORS] = {
		.prot_pte  = L_PTE_PRESENT | L_PTE_YOUNG | L_PTE_DIRTY |
#ifdef CONFIG_ARCH_ARGUS
		/* Vectors are written using domain control, need to be
		 * explicitely write-allowed on Argus
		 */
		L_PTE_WRITE |
#endif
...
#ifdef CONFIG_ARCH_ARGUS
		// We need the set_pmd hook. RMK does not believe any
		// one needs it, thus the discrepancy.
...
void __init memtable_init(struct meminfo *mi)
{
	...
	/* This shouldn't be done here, even in the other ARM archs.
	 * It breaks things because the arch io map is not set up at
	 * this point. If you try to do a printk for example it will 
	 * blow up.
	 */
	flush_tlb_all();
#endif
